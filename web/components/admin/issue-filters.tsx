...
import { input as select } from './styles';
  ...
  return (
    <div className="flex flex-wrap items-end gap-3">
      <label className="block">
        ...
        <select
          className={`${select} min-w-56`} value={competitionId ?? ''} disabled={pending}
          onChange={(e) => {
      ...
      <label className="block">
        ...
        <select
          className={`${select} min-w-36`} value={editionId ?? ''}
      ...
      <label className="block">
        <span className="mb-1.5 block text-xs font-semibold">Issue type</span>
        <select
          className={`${select} min-w-52`} value={kind ?? ''} disabled={pending}
          onChange={(e) =>
            go({
